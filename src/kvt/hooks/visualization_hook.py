...
from .shap_helper import text as shap_text
...
class ShapTextVisualizationHook(VisualizationHookBase):
    def __init__(
        self,
        dirpath,
        experiment_name,
    ...
    def __call__(self, model, dataloader, predictions, targets, logger=None):
        ...
        def f(x):
            x = [dataset._preprocess_input(v) for v in x]
            keys = x[0].keys()
        ...
        explainer = shap.Explainer(f, dataset.tokenizer)

        shap_values = []
        for idx in indices:
            text = dataset.inputs[idx]
            o = explainer([text])  # [1, shap_length, 1]
            shap_values.append(o[0, :, 0])

        out = " ".join([shap_text(v) for v in shap_values])
